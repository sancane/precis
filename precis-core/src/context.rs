...
/// # Arguments
/// * `s`: String label
/// * `offset`: The positoin of the character in the label
/// # Returns
...
/// [Appendinx A.1](https://datatracker.ietf.org/doc/html/rfc5892#appendix-A.1).
...
pub fn rule_zero_width_nonjoiner(s: &str, offset: usize) -> Result<bool, Error> {
    ...
    // RegExpMatch((Joining_Type:{L,D})(Joining_Type:T)*\u200C
    //     (Joining_Type:T)*(Joining_Type:{R,D}))

    // Check all transparent joining type codepoints before {u200C} (0 or more)
    let mut i = offset - 1;
    ...
    // Check all transparent joining type codepoints following {u200C} (0 or more)
...
/// [Appendinx A.2](https://datatracker.ietf.org/doc/html/rfc5892#appendix-A.2).
/// ZERO WIDTH JOINER\
...
/// [Appendinx A.3](https://datatracker.ietf.org/doc/html/rfc5892#appendix-A.3).
/// MIDDLE DOT\
...
/// [Appendinx A.4](https://datatracker.ietf.org/doc/html/rfc5892#appendix-A.4).
...
/// [Appendinx A.5](https://datatracker.ietf.org/doc/html/rfc5892#appendix-A.5).
/// [Appendinx A.6](https://datatracker.ietf.org/doc/html/rfc5892#appendix-A.5).
...
/// [Appendinx A.7](https://datatracker.ietf.org/doc/html/rfc5892#appendix-A.7).
/// KATAKANA MIDDLE DOT\
...
/// [Appendinx A.8](https://datatracker.ietf.org/doc/html/rfc5892#appendix-A.8).
/// ARABIC-INDIC DIGITS\
...
/// [Appendinx A.9](https://datatracker.ietf.org/doc/html/rfc5892#appendix-A.9).
...
mod tests {
    ...
    fn check_rule_zero_width_nonjoiner() {
        // code point at position 0 is not 0x200c
        let label = "A";
        ...
        // Previous cp is neither Virama nor transparent/Joining_Type:{L,D} => false
        let label = "A\u{200c}";
        let res = rule_zero_width_nonjoiner(label, 1);
        assert_eq!(res.is_ok(), true);
        assert_eq!(res.unwrap(), false);

        // Miss Joining_Type:{L,D} before Transparent => Error(Undefined)
        // "(Joining_Type:T)\u200C"
        ...
        // No Joining_Type:{L,D} before Transparent => false
        // 'A'(Joining_Type:T)\u200C
        ...
        // First part of the regExp is complete but fails to meet the second one
        // (Joining_Type:L)(Joining_Type:T)\u200C(Joining_Type:T)
        ...
        // (Joining_Type:L)(Joining_Type:T)\u200C(Joining_Type:T)(Joining_Type:R)
        ...
        // (Joining_Type:L)(Joining_Type:T)\u200C(Joining_Type:T)'A'
        ...
        // (Joining_Type:L)(Joining_Type:T)\u200C'A'
        let label = "\u{a872}\u{5bf}\u{200c}A";
        let res = rule_zero_width_nonjoiner(label, 2);
        assert_eq!(res.is_ok(), true);
        assert_eq!(res.unwrap(), false);

        // 'A'(Joining_Type:T){2}\u200C(Joining_Type:T){4}(Joining_Type:D)
        ...
        // (Joining_Type:D)\u200C(Joining_Type:T)(Joining_Type:D)
        let label = "\u{626}\u{200c}\u{5bf}\u{626}";
        let res = rule_zero_width_nonjoiner(label, 1);
        assert_eq!(res.is_ok(), true);
        assert_eq!(res.unwrap(), true);

        // (Joining_Type:D)\u200C(Joining_Type:D)
        let label = "\u{626}\u{200c}\u{626}";
        let res = rule_zero_width_nonjoiner(label, 1);
        assert_eq!(res.is_ok(), true);
        assert_eq!(res.unwrap(), true);

        // (Joining_Type:D)(Joining_Type:T){2}\u200C(Joining_Type:T){4}(Joining_Type:D)
    ...
    fn check_rule_arabic_indic_digits() {
        ...
        // Label does not contain Extended Arabic-Indic Digits => True
        ...
        // Label contains Extended Arabic-Indic Digits => False
    ...
    fn check_rule_extended_arabic_indic_digits() {
        ...
        // Label does not contain Arabic-Indic Digits => True
